...
_STATE_NONE, _STATE_TALK, _STATE_RUN = 0, 1, 2

class Chemist(Agent):
	def __init__(self, settings, model, agentName, layer, uniqInMap=True):
		super(Chemist, self).__init__(settings, model, agentName, layer, uniqInMap)
		self.state = _STATE_NONE
		self.BEE_SPEED = 3 * float(self.settings.get("rio", "TestAgentSpeed"))
	...
	def start(self):
		...
		#self.agent.move('run', c, self.BEE_SPEED)
		self.talk()
